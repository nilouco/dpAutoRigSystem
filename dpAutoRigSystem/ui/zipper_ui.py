...
class ZipperUI:
    ...
    def create_ui(self, app):
        ...
        cmds.button('zipper_model_bt', label=f"{self.ar.data.lang['i187_load']} {self.ar.data.lang['m152_originalModel']} >>", command=self.load_orig_model, backgroundColor=(1.0, 0.9, 0.4), parent='zipper_model_rcl')
        ...
        cmds.button('zipper_first_bt', label=f"{self.ar.data.lang['i187_load']} {self.ar.data.lang['c114_first']} {self.ar.data.lang['i189_curve']} >>", command=partial(self.app.create_curve_from_edge, 'c114_first'), backgroundColor=(1.0, 0.9, 0.4), parent='zipper_buttons_rcl')
        cmds.textField('zipper_first_tf', editable=False, parent='zipper_buttons_rcl')
        cmds.button('zipper_second_bt', label=f"{self.ar.data.lang['i187_load']} {self.ar.data.lang['c115_second']} {self.ar.data.lang['i189_curve']} >>", command=partial(self.app.create_curve_from_edge, 'c115_second'), backgroundColor=(1.0, 0.9, 0.4), parent='zipper_buttons_rcl')
    ...
    def update_ui(self, curve_name, zipper_id):
        ...
        if zipper_id == 'c114_first':
            ...
            cmds.button('zipper_first_bt', edit=True, label=f"{self.app.first_name} {self.ar.data.lang['i189_curve']}", backgroundColor=(0.3, 0.8, 1.0))
            self.app.first_curve = curve_name
        elif zipper_id == 'c115_second':
            cmds.textField('zipper_second_tf', edit=True, text=curve_name)
            cmds.button('zipper_second_bt', edit=True, label=f"{self.app.second_name} {self.ar.data.lang['i189_curve']}", backgroundColor=(0.3, 0.8, 1.0))
