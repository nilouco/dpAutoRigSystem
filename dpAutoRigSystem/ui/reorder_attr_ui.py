...
class ReorderAttrUI:
    ...
    def create_ui(self, app):
        ...
        width  = 175
        height = 75
        cmds.window(self.ar.data.reorder_attr_win_name, title=f"{self.ar.data.lang['m087_reorderAttr']} {self.ar.data.version}", widthHeight=(width, height), menuBar=False, sizeable=True, minimizeButton=False, maximizeButton=False, menuBarVisible=False, titleBar=True)
        # creating layout:
        cmds.columnLayout('reorder_attr_cl', columnOffset=('left', 30), parent=self.ar.data.reorder_attr_win_name)
        ...
        cmds.showWindow(self.ar.data.reorder_attr_win_name)
