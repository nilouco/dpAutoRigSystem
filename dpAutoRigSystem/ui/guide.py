...
class GuideUI(object):
    ...
    def create_edit_selected_layout(self, standard):
        ...
        cmds.rowLayout('edit_mirror_rig', numberOfColumns=4, columnWidth4=(100, 50, 80, 70), columnAlign=[(1, 'right'), (4, 'right')], adjustableColumn=4, columnAttach=[(1, 'both', 2), (2, 'both', 2), (3, 'both', 2), (4, 'both', 10)], parent="rig_selected_module_cl" )
    ...
    def mirror_layout(self, standard):
        cmds.text(self.ar.data.lang['m010_mirror'], parent='edit_mirror_rig')
        cmds.optionMenu("edit_mirror_om", label='', changeCommand=standard.changeMirror, parent='edit_mirror_rig')
        ...
        cmds.optionMenu("edit_mirror_name_om", label='', changeCommand=standard.changeMirrorName, parent='edit_mirror_rig')
        menu_name_item_text = ""
    ...
    def rig_it_button(self, standard):
        cmds.button('edit_rig_it_bt', label="Rig", command=standard.rig_me, backgroundColor=(1.0, 1.0, 0.7), parent='edit_mirror_rig')
    ...
    def flip_layout(self, standard):
        ...
        if 'flip' in cmds.listAttr(standard.guide_base):
            ...
            flipValue = cmds.getAttr(standard.guide_base+".flip")
            self.flipCB = cmds.checkBox(label="flip", value=flipValue, changeCommand=standard.changeFlip, parent=self.flipLayout)
    ...
    def degree_layout(self, standard):
        ...
        if 'degree' in cmds.listAttr(standard.guide_base):
            self.degreeColumn = cmds.rowLayout('degreeColumn', numberOfColumns=3, columnWidth3=(100, 100, 70), columnAlign=[(1, 'right'), (3, 'right')], adjustableColumn=3, columnAttach=[(1, 'both', 2), (2, 'both', 2), (3, 'both', 2)], parent="rig_selected_module_cl" )
            cmds.text(self.ar.data.lang['i119_curveDegree'], parent=self.degreeColumn)
            cmds.optionMenu(f"{standard.number_name}_curve_degree_om", label='', changeCommand=partial(self.change_curve_degree, standard), parent=self.degreeColumn)
    ...
    def facial_layout(self, standard):
        ...
        if 'facial' in cmds.listAttr(standard.guide_base):
            ...
            if not facialValue:
                collapsed = True
            # facial frame layout
            cmds.frameLayout(f"{standard.number_name}_facial_fl", label=self.ar.data.lang['m139_facialCtrlsAttr'], collapsable=True, collapse=collapsed, enable=facialValue, parent="rig_selected_module_cl")
            cmds.rowColumnLayout(f"{standard.number_name}_facial_rcl", numberOfColumns=2, columnWidth=[(1, 70), (2, 300)], columnAlign=[(1, 'left'), (2, 'left')], columnAttach=[(1, 'left', 10), (2, 'left', 20)], parent=f"{standard.number_name}_facial_fl")
            ...
            self.facialTypeLayout = cmds.columnLayout('facialTypeLayout', parent=f"{standard.number_name}_facial_fl")
    ...
    def reorient_layout(self, standard):
        if 'reorient' in cmds.listAttr(standard.guide_base):
            if standard.name == "FkLine":
                cmds.button('edit_reorient_fk_bt', label=self.ar.data.lang["m022_reOrient"], annotation=self.ar.data.lang["m023_reOrientDesc"], command=standard.reOrientGuideButton, parent="flipLayout")
