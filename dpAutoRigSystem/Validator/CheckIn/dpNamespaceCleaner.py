                    ...
                    if namespace != "UI" and namespace != "shared":
                        ...
                        for type in types:
                            if type.find("_dpAR_") != -1:
                                ...
                                namespaceWithoutGuidesList.append(namespace)
                for item in namespaceWithoutGuidesList:
                    if item in namespaceWithGuidesList:
                        namespaceWithoutGuidesList.remove(item)
                namespaceWithGuidesList = list(set(namespaceWithGuidesList))
                namespaceWithoutGuidesList = list(set(namespaceWithoutGuidesList))
                namespaceListToClean = namespaceWithGuidesList + namespaceWithoutGuidesList
                
        if namespaceListToClean:
            print("namespaceListToClean =========", namespaceListToClean)
            progressAmount = 0
                ...
                if self.verifyMode:
                    ...
                    try:
                        if namespace in namespaceWithGuidesList:
                            print("namespceWithoutGuides ==", namespace)
