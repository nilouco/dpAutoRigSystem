...
class Renamer():
    ...
    def renamerUI(self, *args):
        ...
        renamerWidth = 530
        renamerHeight = 280
        dpRenamerWin = cmds.window('dpRenamerWin', title=self.dpUIinst.langDic[self.dpUIinst.langName]['m214_renamer']+' - v'+str(DP_RENAMER_VERSION), width=renamerWidth, height=renamerHeight, sizeable=False, minimizeButton=False, maximizeButton=False)
        # UI elements:
        ...
        self.selectRB = cmds.radioButtonGrp('selectRB', labelArray2=[self.dpUIinst.langDic[self.dpUIinst.langName]["i266_selected"], self.dpUIinst.langDic[self.dpUIinst.langName]["m216_hierarchy"]], numberOfRadioButtons=2, select=self.selOption, changeCommand=self.changeSelOption, parent=fieldsLayout)
        cmds.separator(style="in", height=20, parent=fieldsLayout)
        self.sequenceCB = cmds.checkBox('sequenceCB', label=self.dpUIinst.langDic[self.dpUIinst.langName]['m220_sequence'], changeCommand=self.sequenceCBChange, value=False, parent=fieldsLayout)
        self.sequenceTFG = cmds.textFieldGrp('sequenceTFG', label=self.dpUIinst.langDic[self.dpUIinst.langName]['m222_name'], textChangedCommand=self.nameChange, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 100)], adjustableColumn2=True, parent=fieldsLayout)
        self.startIFG = cmds.intFieldGrp('startIFG', label=self.dpUIinst.langDic[self.dpUIinst.langName]['c110_start'], changeCommand=self.refreshPreview, value1=self.start, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 100)], adjustableColumn2=True, parent=fieldsLayout)
        self.paddingIFG = cmds.intFieldGrp('paddingIFG', label=self.dpUIinst.langDic[self.dpUIinst.langName]['m221_padding'], changeCommand=self.refreshPreview, value1=self.padding, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 100)], adjustableColumn2=True, parent=fieldsLayout)
        cmds.separator(style="in", height=20, parent=fieldsLayout)
        ...
        self.prefixCB = cmds.checkBox('prefixCB', label=self.dpUIinst.langDic[self.dpUIinst.langName]['i144_prefix'], changeCommand=self.refreshPreview, value=False, parent=prePosLayout)
        self.suffixCB = cmds.checkBox('suffixCB', label=self.dpUIinst.langDic[self.dpUIinst.langName]['m217_suffix'], changeCommand=self.refreshPreview, value=False, parent=prePosLayout)
        ...
        cmds.separator(style="in", height=20, parent=fieldsLayout)
        self.searchReplaceCB = cmds.checkBox('searchReplaceCB', label=self.dpUIinst.langDic[self.dpUIinst.langName]['m218_search']+" - "+self.dpUIinst.langDic[self.dpUIinst.langName]['m219_replace'], changeCommand=self.searchReplaceCBChange, value=False, parent=fieldsLayout)
        self.searchTFG = cmds.textFieldGrp('searchTFG', label=self.dpUIinst.langDic[self.dpUIinst.langName]['i036_from'], textChangedCommand=self.searchChange, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 136)], adjustableColumn2=True, parent=fieldsLayout)
        self.replaceTFG = cmds.textFieldGrp('replaceTFG', label=self.dpUIinst.langDic[self.dpUIinst.langName]['i037_to'], textChangedCommand=self.searchChange, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 136)], adjustableColumn2=True, parent=fieldsLayout)
        # loaded items
        itemsLayout = cmds.columnLayout('itemsLayout', adjustableColumn=True, width=300, parent=mainLayout)
        cmds.text(label=self.dpUIinst.langDic[self.dpUIinst.langName]['m223_preview'], align="center", height=20, font="boldLabelFont", parent=itemsLayout)
        ...
        cmds.text(label=self.dpUIinst.langDic[self.dpUIinst.langName]['i276_current'], parent=scrollsLayout)
        cmds.text(label=self.dpUIinst.langDic[self.dpUIinst.langName]['m224_rename']+" "+self.dpUIinst.langDic[self.dpUIinst.langName]['i037_to'], parent=scrollsLayout)
        ...
        cmds.button('runRenamerBT', label=self.dpUIinst.langDic[self.dpUIinst.langName]['m224_rename'], command=self.runRenamerByUI, parent=footerLayout)
        # calling UI:
            ...
            if self.selOption == 2: #Hierarchy
                for item in self.originalList:
                    try:
                        ...
                        mel.eval("warning \""+self.dpUIinst.langDic[self.dpUIinst.langName]['i075_moreOne']+' '+self.dpUIinst.langDic[self.dpUIinst.langName]['i076_sameName']+"\";")
            ...
            if self.previewList:
                for i, item in enumerate(self.originalList):
                    ...
                    if cmds.objExists(item):
                        ...
                        mel.eval("warning \""+self.dpUIinst.langDic[self.dpUIinst.langName]['v005_cantFix']+" "+item+"\";")
            self.resetUI()
            self.refreshPreview()
        else:
            mel.eval("warning \""+self.dpUIinst.langDic[self.dpUIinst.langName]['m225_selectAnything']+"\";")
