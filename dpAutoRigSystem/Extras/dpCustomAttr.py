...
class CustomAttr(object):
    ...
    def mainUI(self, *args):
        ...
        cmds.window(self.mainWindowName, title=self.dpUIinst.langDic[self.dpUIinst.langName]['m212_customAttr']+" "+str(DP_CUSTOMATTR_VERSION), widthHeight=(customAttributes_winWidth, customAttributes_winHeight), menuBar=False, sizeable=True, minimizeButton=True, maximizeButton=False)
        ...
        cmds.text("headerTxt", label=self.dpUIinst.langDic[self.dpUIinst.langName]['i267_customHeader']+' "'+ATTR_START+'"', align="left", height=30, font='boldLabelFont', parent=mainLayout)
        # filter
        filterLayout = cmds.columnLayout("filterLayout", adjustableColumn=True, parent=mainLayout)
        self.itemFilterTFG = cmds.textFieldButtonGrp("itemFilterTFG", label=self.dpUIinst.langDic[self.dpUIinst.langName]['i268_filterByName'], text="", buttonLabel=self.dpUIinst.langDic[self.dpUIinst.langName]['m004_select']+" "+self.dpUIinst.langDic[self.dpUIinst.langName]['i211_all'], buttonCommand=self.selectAllTransforms, changeCommand=self.filterByName, adjustableColumn=2, parent=filterLayout)
        ...
        cmds.button("addButton", label=self.dpUIinst.langDic[self.dpUIinst.langName]['i063_skinAddBtn'], backgroundColor=(0.6, 0.6, 0.6), width=70, command=self.addAttrUI, parent=buttonLayout)
        cmds.button("removeButton", label=self.dpUIinst.langDic[self.dpUIinst.langName]['i064_skinRemBtn'], backgroundColor=(0.4, 0.4, 0.4), width=70, command=self.removeAttrUI, parent=buttonLayout)
        cmds.button("updateIDButton", label=self.dpUIinst.langDic[self.dpUIinst.langName]['i089_update']+" "+ATTR_DPID, backgroundColor=(0.5, 0.5, 0.5), width=100, command=self.updateID, parent=buttonLayout)
        # call window
    ...
    def addAttrUI(self, *args):
        ...
        add_winWidth  = 220
        add_winHeight = 260
        cmds.window(self.addWindowName, title=self.dpUIinst.langDic[self.dpUIinst.langName]['m212_customAttr']+" "+str(DP_CUSTOMATTR_VERSION), widthHeight=(add_winWidth, add_winHeight), menuBar=False, sizeable=True, minimizeButton=True, maximizeButton=False)
        addAttrLayout = cmds.columnLayout('addAttrLayout', adjustableColumn=True, columnOffset=("both", 10))
        cmds.text("headerAddTxt", label=self.dpUIinst.langDic[self.dpUIinst.langName]['i045_add']+" "+self.dpUIinst.langDic[self.dpUIinst.langName]['m212_customAttr'], align="left", height=30, font='boldLabelFont', parent=addAttrLayout)
        ...
        cmds.text("customAddTxt", label=self.dpUIinst.langDic[self.dpUIinst.langName]['m212_customAttr']+":", align="left", height=30, parent=addAttrLayout)
        self.addCustomAttrTFG = cmds.textFieldButtonGrp("addCustumAttrTFG", label="", text="", buttonLabel=self.dpUIinst.langDic[self.dpUIinst.langName]['i045_add'], buttonCommand=partial(self.addAttr, "custom", False), adjustableColumn=2, columnWidth=[(1, 0), (2, 50), (3, 30)], parent=addAttrLayout)
    ...
    def removeAttrUI(self, *args):
        ...
        cmds.window(self.removeWindowName, title=self.dpUIinst.langDic[self.dpUIinst.langName]['m212_customAttr']+" "+str(DP_CUSTOMATTR_VERSION), widthHeight=(remove_winWidth, remove_winHeight), menuBar=False, sizeable=True, minimizeButton=True, maximizeButton=False)
        removeAttrLayout = cmds.columnLayout('removeAttrLayout', adjustableColumn=True, columnOffset=("both", 10))
        cmds.text("headerRemoveTxt", label=self.dpUIinst.langDic[self.dpUIinst.langName]['i046_remove']+" "+self.dpUIinst.langDic[self.dpUIinst.langName]['m212_customAttr'], align="left", height=30, font='boldLabelFont', parent=removeAttrLayout)
        ...
        if toRemoveAttrList:
            ...
            cmds.text("noCustomAttrTxt", label=self.dpUIinst.langDic[self.dpUIinst.langName]['i062_notFound']+" "+self.dpUIinst.langDic[self.dpUIinst.langName]['m212_customAttr'])
