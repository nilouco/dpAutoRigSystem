...
class Wheel(standard.BaseStandard):
    ...
    def create_guide_elements(self):
        ...
        """
        # locators
        self.guide_center_loc = self.ar.ctrls.create_joint_locator(ctrl_name=f"{self.name_guide}_CenterLoc", r=0.6, d=1, rot=(90, 0, 90), guide=True)
        self.guide_front_loc = self.ar.ctrls.create_controller('id_059_AimLoc', ctrl_name=f"{self.name_guide}_FrontLoc", r=0.3, d=1, rot=(0, 0, 90))
        self.guide_inside_loc = self.ar.ctrls.create_curve_locator(ctrl_name=f"{self.name_guide}_InsideLoc", r=0.2, d=1, guide=True)
        self.guide_outside_loc = self.ar.ctrls.create_curve_locator(ctrl_name=f"{self.name_guide}_OutsideLoc", r=0.2, d=1, guide=True)
        # joints
        self.line_center = cmds.joint(name=f"{self.name_guide}_JGuideCenter", radius=0.001)
        self.line_front = cmds.joint(name=f"{self.name_guide}_JGuideFront", radius=0.001)
        self.line_inside = cmds.joint(name=f"{self.name_guide}_JGuideInside", radius=0.001)
        self.line_outside = cmds.joint(name=f"{self.name_guide}_JGuideOutside", radius=0.001)
        # setup
        self.ar.utils.set_template([self.line_center, self.line_front, self.line_inside, self.line_outside])
        front_loc_pos_pma = cmds.createNode('plusMinusAverage', name=f"{self.guide_front_loc}_Pos_PMA")
        inverse_radius_md = cmds.createNode('multiplyDivide', name=f"{self.guide_base}_Radius_Inv_MD")
        cmds.setAttr(f"{self.guide_front_loc}.translateX", 1.3)
        cmds.setAttr(f"{self.guide_inside_loc}.translateZ", 0.3)
        ...
        # edit
        cmds.connectAttr(f"{self.radius_ctrl}.translateX", f"{front_loc_pos_pma}.input1D[1]")
        cmds.connectAttr(f"{front_loc_pos_pma}.output1D", f"{self.guide_front_loc}.translateX")
        cmds.connectAttr(f"{self.radius_ctrl}.translateX", f"{inverse_radius_md}.input1X")
        cmds.connectAttr(f"{inverse_radius_md}.outputX", f"{self.guide_inside_loc}.translateY")
        cmds.connectAttr(f"{inverse_radius_md}.outputX", f"{self.guide_outside_loc}.translateY")
        ...
        cmds.parentConstraint(self.guide_center_loc, self.line_center, maintainOffset=False, name=f"{self.line_center}_PaC")
        cmds.parentConstraint(self.guide_front_loc, self.line_front, maintainOffset=False, name=f"{self.line_front}_PaC")
        cmds.parentConstraint(self.guide_inside_loc, self.line_inside, maintainOffset=False, name=f"{self.guide_inside_loc}_PaC")
        cmds.parentConstraint(self.guide_outside_loc, self.line_outside, maintainOffset=False, name=f"{self.guide_outside_loc}_PaC")
    ...
    def rig_me(self, *args):
        ...
        if cmds.objExists(self.guide_base):
            ...
            for s, side in enumerate(self.sides):
                ...
                self.guide_center_loc = f"{side}{self.number_name}_Guide_CenterLoc"
                self.guide_front_loc = f"{side}{self.number_name}_Guide_FrontLoc"
                self.guide_inside_loc = f"{side}{self.number_name}_Guide_InsideLoc"
                self.guide_outside_loc = f"{side}{self.number_name}_Guide_OutsideLoc"
                self.guide_radius = f"{side}{self.number_name}_Guide_Base_RadiusCtrl"
                
                # create a joint:
                cmds.select(clear=True)
                # center joint:
                center_joint = cmds.joint(name=f"{side}{self.number_name}_{self.ar.data.lang['m156_wheel']}_Jnt", scaleCompensate=False)
                ...
                self.ar.ctrls.transfer_shape(delete_source = True, clear_dest_shapes=False, source_item=self.ar.ctrls.create_controller('Clip', f"{side}{self.number_name}_{self.ar.data.lang['m106_clip']}_Ctrl", r=self.radius*0.2, d=self.curve_degree, rot = (0, 0, 0) ), destinations=[wheel_ctrl], keep_color=False)
                self.ar.ctrls.transfer_shape(delete_source = True, clear_dest_shapes=False, source_item=self.ar.ctrls.create_controller('Clip', f"{side}{self.number_name}_{self.ar.data.lang['m106_clip']}_Ctrl", r=self.radius*0.2, d=self.curve_degree, rot = (0, 0, 90) ), destinations=[wheel_ctrl], keep_color=False)
                ...
                main_ctrl = self.ar.ctrls.create_controller('id_061_WheelMain', f"{side}{self.number_name}_{self.ar.data.lang['c058_main']}_Ctrl", r=self.radius*0.4, d=self.curve_degree, guide_source=f"{self.name_guide}_CenterLoc", parent_tag=wheel_ctrl)
                inside_ctrl = self.ar.ctrls.create_controller('id_062_WheelPivot', f"{side}{self.number_name}_{self.ar.data.lang['c011_revFoot_B'].capitalize()}_Ctrl", r=self.radius*0.2, d=self.curve_degree, rot=(0, 90, 0), guide_source=f"{self.name_guide}_InsideLoc", parent_tag=main_ctrl)
                outside_ctrl = self.ar.ctrls.create_controller('id_062_WheelPivot', f"{side}{self.number_name}_{self.ar.data.lang['c010_revFoot_A'].capitalize()}_Ctrl", r=self.radius*0.2, d=self.curve_degree, rot=(0, 90, 0), guide_source=f"{self.name_guide}_OutsideLoc", parent_tag=main_ctrl)
                ...
                to_steering_grp = cmds.group(inside_ctrl, name=f"{side}{self.number_name}_{self.ar.data.lang['c070_steering'].capitalize()}_Grp")
                cmds.addAttr(to_steering_grp, longName=self.ar.data.lang['c070_steering'], attributeType='bool', keyable=True)
                cmds.addAttr(to_steering_grp, longName=f"{self.ar.data.lang['c070_steering']}{self.ar.data.lang['m151_invert']}", attributeType='bool', keyable=True)
                cmds.setAttr(f"{to_steering_grp}.{self.ar.data.lang['c070_steering']}", 1)
                ...
                wheel_auto_grp = cmds.rename(wheel_auto_grp, f"{side}{self.number_name}_{self.ar.data.lang['m156_wheel']}_Auto_Grp")
                
                # fixing flip mirror:
                if s == 1 and cmds.getAttr(f"{self.guide_base}.flip") == 1:
                    for zero_grp in zeros:
                        cmds.setAttr(f"{zero_grp}.scaleX", -1)
                        cmds.setAttr(f"{zero_grp}.scaleY", -1)
                        cmds.setAttr(f"{zero_grp}.scaleZ", -1)
                
                cmds.addAttr(wheel_ctrl, longName='scaleCompensate', attributeType='short', minValue=0, maxValue=1, defaultValue=1, keyable=False)
                cmds.setAttr(f"{wheel_ctrl}.scaleCompensate", 1, channelBox=True)
                cmds.connectAttr(f"{wheel_ctrl}.scaleCompensate", f"{center_joint}.segmentScaleCompensate", force=True)
                cmds.addAttr(main_ctrl, longName='scaleCompensate', attributeType='short', minValue=0, maxValue=1, defaultValue=1, keyable=False)
                cmds.setAttr(f"{main_ctrl}.scaleCompensate", 1, channelBox=True)
                cmds.connectAttr(f"{main_ctrl}.scaleCompensate", f"{main_joint}.segmentScaleCompensate", force=True)
                ...
                # grouping:
                cmds.parentConstraint(wheel_ctrl, center_joint, maintainOffset=False, name=f"{center_joint}_PaC")
                cmds.scaleConstraint(wheel_ctrl, center_joint, maintainOffset=True, name=f"{center_joint}_ScC")
                ...
                cmds.addAttr(wheel_ctrl, longName=f"{self.ar.data.lang['i037_to']}{self.ar.data.lang['c070_steering'].capitalize()}", attributeType='float', defaultValue=0, keyable=False)
                cmds.addAttr(wheel_ctrl, longName=f"{self.ar.data.lang['c070_steering']}{self.ar.data.lang['c053_invert'].capitalize()}", attributeType='long', min=0, max=1, defaultValue=1, keyable=False)
                ...
                start_frame_value = cmds.getAttr(f"{self.guide_base}.startFrame")
                steering_value = cmds.getAttr(f"{self.guide_base}.steering")
                show_ctrls_value = cmds.getAttr(f"{self.guide_base}.showControls")
                cmds.setAttr(f"{wheel_ctrl}.{self.ar.data.lang['c068_startFrame']}", start_frame_value, channelBox=True)
                cmds.setAttr(f"{wheel_ctrl}.{self.ar.data.lang['c070_steering']}", steering_value, channelBox=True)
                cmds.setAttr(f"{wheel_ctrl}.{self.ar.data.lang['c021_showControls']}", show_ctrls_value, channelBox=True)
                cmds.setAttr(f"{wheel_ctrl}.{self.ar.data.lang['c070_steering']}{self.ar.data.lang['c053_invert'].capitalize()}", 1, channelBox=True)
                self.ar.ctrls.set_default_value(wheel_ctrl, f"{self.ar.data.lang['c070_steering']}{self.ar.data.lang['c053_invert'].capitalize()}", 1)
                cmds.setAttr(f"{wheel_ctrl}.{self.ar.data.lang['c093_tryKeepUndo']}", 1, channelBox=True)
                if s == 1 and cmds.getAttr(f"{self.guide_base}.flip") == 1:
                    cmds.setAttr(f"{wheel_ctrl}.{self.ar.data.lang['c070_steering']}{self.ar.data.lang['c053_invert'].capitalize()}", 0)
                    self.ar.ctrls.set_default_value(wheel_ctrl, f"{self.ar.data.lang['c070_steering']}{self.ar.data.lang['c053_invert'].capitalize()}", 0)
                # set default values:
                ...
                recept_steering_md = cmds.createNode('multiplyDivide', name=f"{side}{self.number_name}_{self.ar.data.lang['c070_steering'].capitalize()}_MD")
                inverse_steering_md = cmds.createNode('multiplyDivide', name=f"{side}{self.number_name}_{self.ar.data.lang['c070_steering'].capitalize()}_Inv_MD")
                inverse_steering_cnd = cmds.createNode('condition', name=f"{side}{self.number_name}_{self.ar.data.lang['c070_steering'].capitalize()}_Inv_Cnd")
                cmds.setAttr(f"{inverse_steering_cnd}.colorIfTrueR", 1)
                cmds.setAttr(f"{inverse_steering_cnd}.colorIfFalseR", -1)
                cmds.connectAttr(f"{wheel_ctrl}.{self.ar.data.lang['i037_to']}{self.ar.data.lang['c070_steering'].capitalize()}", f"{recept_steering_md}.input1X", force=True)
                cmds.connectAttr(f"{wheel_ctrl}.{self.ar.data.lang['c070_steering']}", f"{recept_steering_md}.input2X", force=True)
                cmds.connectAttr(f"{recept_steering_md}.outputX", f"{inverse_steering_md}.input1X", force=True)
                cmds.connectAttr(f"{inverse_steering_cnd}.outColorR", f"{inverse_steering_md}.input2X", force=True)
                cmds.connectAttr(f"{wheel_ctrl}.{self.ar.data.lang['c070_steering']}{self.ar.data.lang['c053_invert'].capitalize()}", f"{inverse_steering_cnd}.firstTerm", force=True)
                cmds.connectAttr(f"{inverse_steering_md}.outputX", f"{to_steering_grp}.rotateY", force=True)
                # create locators (frontLoc to get direction and oldLoc to store wheel old position):
                front_loc = cmds.spaceLocator(name=f"{side}{self.number_name}_{self.ar.data.lang['m156_wheel']}_Front_Loc")[0]
                old_loc = cmds.spaceLocator(name=f"{side}{self.number_name}_{self.ar.data.lang['m156_wheel']}_Old_Loc")[0]
                ...
                cmds.setAttr(f"{front_loc}.visibility", 0, lock=True)
                cmds.setAttr(f"{old_loc}.visibility", 0, lock=True)
                ...
                wheel_auto_grp_loc = cmds.spaceLocator(name=f"{side}{self.number_name}_{self.ar.data.lang['m156_wheel']}_Auto_Loc")[0]
                cmds.pointConstraint(wheel_auto_grp, wheel_auto_grp_loc, maintainOffset=False, name=f"{wheel_auto_grp_loc}_PoC")
                ...
                # expression:
                exp_node = cmds.expression(name=f"{side}{self.number_name}_{self.ar.data.lang['m156_wheel']}_Exp", object=front_loc, string=exp_text)
                self.ar.ctrls.set_lock_hide([front_loc, wheel_auto_grp_loc], ['tx', 'ty', 'tz', 'rx', 'ry', 'rz', 'sx', 'sy', 'sz', 'v'])
                
                # deformers:
                loaded_geo = cmds.getAttr(f"{self.guide_base}.geo")
                
                # geometry holder:
                geo_holder = cmds.polyCube(name=f"{side}{self.number_name}_{self.ar.data.lang['c046_holder']}_Geo", constructionHistory=False)[0]
                cmds.matchTransform(geo_holder, self.guide_center_loc, position=True, rotation=True)
                cmds.setAttr(f"{geo_holder}.visibility", 0, lock=True)
                
                # skinning:
                skincluster_node = cmds.skinCluster(center_joint, geo_holder, toSelectedBones=True, dropoffRate=4.0, maximumInfluences=3, skinMethod=0, normalizeWeights=1, removeUnusedInfluence=False, name=f"{side}{self.number_name}_{self.ar.data.lang['c046_holder']}_SC")[0]
                bindpose_node = cmds.listConnections(f"{skincluster_node}.bindPose", destination=False, source=True)
                cmds.rename(bindpose_node, f"{side}{self.number_name}_{self.ar.data.lang['c046_holder']}_BP")
                    ...
                    try:
                        ...
                        for item in cmds.listRelatives(loaded_geo, children=True, allDescendents=True) or []:
                            ...
                            if item_type == 'mesh' or item_type == 'nurbsSurface':
                                try:
                                    skincluster_name = f"{self.ar.naming.extract_suffix(item)}_SC"
                ...
                # lattice:
                lattice_items = cmds.lattice(geo_holder, divisions=(6, 6, 6), outsideLattice=2, outsideFalloffDistance=1, position=(0, 0, 0), scale=(self.radius*2, self.radius*2, self.radius*2), name=f"{side}{self.number_name}_FFD") #[deformer, lattice, base]
                cmds.scale(self.radius*2, self.radius*2, self.radius*2, lattice_items[2])
                # clusters:
                upper_clusters = cmds.cluster(f"{lattice_items[1]}.pt[0:5][4:5][0:5]", relative=True, name=f"{side}{self.number_name}_{self.ar.data.lang['c044_upper']}_Cls") #[deform, handle]
                middle_clusters = cmds.cluster(f"{lattice_items[1]}.pt[0:5][2:3][0:5]", relative=True, name=f"{side}{self.number_name}_{self.ar.data.lang['m033_middle']}_Cls") #[deform, handle]
                lower_clusters = cmds.cluster(f"{lattice_items[1]}.pt[0:5][0:1][0:5]", relative=True, name=f"{side}{self.number_name}_{self.ar.data.lang['c045_lower']}_Cls") #[deform, handle]                
                cluster_grps = self.ar.utils.create_zero_out([upper_clusters[1], middle_clusters[1], lower_clusters[1]])
                cluster_grp = cmds.group(cluster_grps, name=f"{side}{self.number_name}_Clusters_Grp")
                
                # deform controls:
                upper_def_ctrl = self.ar.ctrls.create_controller('id_063_WheelDeform', f"{side}{self.number_name}_{self.ar.data.lang['c044_upper']}_Ctrl", r=self.radius*0.5, d=self.curve_degree, guide_source=f"{self.name_guide}_CenterLoc", parent_tag=wheel_ctrl)
                middle_def_ctrl = self.ar.ctrls.create_controller('id_064_WheelMiddle', f"{side}{self.number_name}_{self.ar.data.lang['m033_middle']}_Ctrl", r=self.radius*0.5, d=self.curve_degree, guide_source=f"{self.name_guide}_CenterLoc", parent_tag=wheel_ctrl)
                lower_def_ctrl = self.ar.ctrls.create_controller('id_063_WheelDeform', f"{side}{self.number_name}_{self.ar.data.lang['c045_lower']}_Ctrl", r=self.radius*0.5, d=self.curve_degree, rot=(0, 0, 180), guide_source=f"{self.name_guide}_CenterLoc", parent_tag=wheel_ctrl)
                def_ctrl_grps = self.ar.utils.create_zero_out([upper_def_ctrl, middle_def_ctrl, lower_def_ctrl])
                def_ctrl_grp = cmds.group(def_ctrl_grps, name=f"{side}{self.number_name}_Ctrl_Grp")
                ...
                outside_dist = cmds.getAttr(f"{self.guide_outside_loc}.translateZ")
                if s == 1 and cmds.getAttr(f"{self.guide_base}.flip") == 1:
                ...
                def_grp = cmds.group(lattice_items[1], lattice_items[2], cluster_grp, name=f"{side}{self.number_name}_Deform_Grp")
                cmds.parentConstraint(main_ctrl, def_grp, maintainOffset=True, name=f"{def_grp}_PaC")
                cmds.scaleConstraint(main_ctrl, def_grp, maintainOffset=True, name=f"{def_grp}_ScC")
                cmds.parent(def_ctrl_grp, main_ctrl)
                cmds.connectAttr(f"{wheel_ctrl}.{self.ar.data.lang['c021_showControls']}", f"{def_ctrl_grp}.visibility", force=True)
                ...
                self.to_ids.extend([recept_steering_md, inverse_steering_md, inverse_steering_cnd, exp_node, geo_holder, skincluster_node, f"{side}{self.number_name}_{self.ar.data.lang['c046_holder']}_BP"])
