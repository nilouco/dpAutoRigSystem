...
class Finger(standard.BaseStandard):
    ...
    def create_guide_elements(self):
        ...
        """
        # locators
        self.guide_base_joint_loc = self.ar.ctrls.cvLocator(ctrlName=self.name_guide+"_JointLoc0", r=0.2, d=1, guide=True)
        self.guide_joint_1_loc = self.ar.ctrls.cvJointLoc(ctrlName=self.name_guide+"_JointLoc1", r=0.3, d=1, guide=True)
        ...
        if cmds.objExists(self.guide_base):
            ...
            for s, side in enumerate(self.sides):
                ...
                for n in range(0, self.n_joints+1):
                    ...
                    # create a control:
                    if n == 1:
                        finger_ctrl = self.ar.ctrls.cvControl("id_015_FingerMain", ctrlName=side+self.number_name+"_%02d_Ctrl"%(n), r=(self.radius * 2.0), d=self.curve_degree, rot=(0, 0, -90), guideSource=self.name_guide+"_JointLoc"+str(n), parentTag=self.controllers[0])
                        ...
                        finger_ctrl = self.ar.ctrls.cvControl("id_016_FingerFk", ctrlName=side+self.number_name+"_%02d_Ctrl"%(n), r=self.radius, d=self.curve_degree, guideSource=self.name_guide+"_JointLoc"+str(n), parentTag=self.get_parent_to_tag(self.controllers))
                        cmds.setAttr(finger_ctrl+".rotateOrder", 1)
                        if n == self.n_joints:
                ...
                if self.n_joints >= 2:
                    ...
                    ik_ctrl = self.ar.ctrls.cvControl("id_017_FingerIk", ctrlName=side+self.number_name+"_Ik_Ctrl", r=(self.radius * 0.3), d=self.curve_degree, guideSource=self.name_guide+"_JointEnd", parentTag=self.controllers[1])
