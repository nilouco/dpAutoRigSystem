...
class Single(standard.BaseStandard):
    ...
    def create_guide_elements(self):
        ...
        cmds.scaleConstraint(self.guide_end_loc, self.line_end, maintainOffset=False, name=f"{self.line_end}_ScC")
    ...
    def change_indirectskin(self, value, *args):
        ...
        """
        cmds.setAttr(f"{self.guide_base}.indirectSkin", value)
        if value == 0:
            cmds.setAttr(f"{self.guide_base}.holder", 0)
            cmds.setAttr(f"{self.guide_base}.sdkLocator", 0)
    ...
    def rig_me(self, *args):
        ...
        if cmds.objExists(self.guide_base):
            ...
            for s, side in enumerate(self.sides):
                ...
                # create a joint:
                jnt = cmds.joint(name=f"{side}{self.number_name}_Jnt", scaleCompensate=False)
                ...
                # create a control:
                if not self.get_guide_attr('indirectSkin') and self.curve_degree == 0:
                    self.curve_degree = 1
                ...
                if self.ar.data.lang['c058_main'] in self.number_name:
                    ...
                    if self.ar.data.lang['c045_lower'] in self.number_name:
                        ...
                        if '00' in self.number_name:
                            ...
                            indirectskin_rot=(0, 0, -90)
                single_ctrl = self.ar.ctrls.create_controller(ctrl_type_id, f"{side}{self.number_name}_Ctrl", r=self.radius, d=self.curve_degree, rot=indirectskin_rot, head_def=cmds.getAttr(f"{self.base}.deformedBy"), guide_source=f"{self.name_guide}_JointLoc1")
                self.ar.utils.set_origined_from_attr(single_ctrl, f"{self.base};{self.guide};{self.guide_end_loc};{self.guide_radius}")
                ...
                cmds.setAttr(f"{single_ctrl}.visibility", keyable=False)
                # fixing flip mirror:
                if s == 1 and cmds.getAttr(f"{self.guide_base}.flip") == 1:
                    cmds.setAttr(f"{single_ctrl_zero}.scaleX", -1)
                    cmds.setAttr(f"{single_ctrl_zero}.scaleY", -1)
                    cmds.setAttr(f"{single_ctrl_zero}.scaleZ", -1)
                ...
                if self.get_guide_attr('indirectSkin'):
                    ...
                    for attr in self.ar.data.transform_attrs[:-1]:
                        cmds.connectAttr(f"{single_ctrl}.{attr}", f"{jnt}.{attr}", force=True)
                    ...
                    if s == 1 and cmds.getAttr(f"{self.guide_base}.flip") == 1:
                        ...
                        for axis in self.ar.data.axes:
                            cmds.setAttr(f"{inv_md}.input2{axis}", -1)
                            cmds.connectAttr(f"{single_ctrl}.translate{axis}", f"{inv_md}.input1{axis}", force=True)
                            cmds.connectAttr(f"{inv_md}.output{axis}", f"{jnt}.translate{axis}", force=True)
                    if self.get_guide_attr('holder'):
                        cmds.delete(f"{single_ctrl}0Shape", shape=True)
                        single_ctrl = cmds.rename(single_ctrl, f"{single_ctrl}_{self.ar.data.lang['c046_holder']}_Grp")
                        ...
                        jnt = cmds.rename(jnt, jnt.replace('_Jnt', f"_{self.ar.data.lang['c046_holder']}_Jis"))
                        ...
                        if self.get_guide_attr('sdkLocator') and not self.ar.data.lang['c058_main'] in self.number_name:
                            ...
                            cmds.connectAttr(f"{sdk_loc}.translateX", f"{sdk_loc_md}.input1X", force=True)
                            cmds.connectAttr(f"{sdk_loc}.translateY", f"{sdk_loc_md}.input1Y", force=True)
                            cmds.connectAttr(f"{sdk_loc}.translateZ", f"{sdk_loc_md}.input1Z", force=True)
                            cmds.connectAttr(f"{sdk_loc}.intensityX", f"{sdk_loc_md}.input2X", force=True)
                            cmds.connectAttr(f"{sdk_loc}.intensityY", f"{sdk_loc_md}.input2Y", force=True)
                            cmds.connectAttr(f"{sdk_loc}.intensityZ", f"{sdk_loc_md}.input2Z", force=True)
                            cmds.connectAttr(f"{sdk_loc_md}.outputX", f"{sdk_loc_grp}.translateX", force=True)
                            cmds.connectAttr(f"{sdk_loc_md}.outputY", f"{sdk_loc_grp}.translateY", force=True)
                            cmds.connectAttr(f"{sdk_loc_md}.outputZ", f"{sdk_loc_grp}.translateZ", force=True)
                            cmds.addAttr(single_ctrl, longName="displayLocator", attributeType='bool', keyable=False)
                            cmds.setAttr(f"{single_ctrl}.displayLocator", 0, channelBox=True)
                            cmds.connectAttr(f"{single_ctrl}.displayLocator", f"{sdk_loc}.visibility", force=True)
                            cmds.setAttr(f"{sdk_loc}.visibility", lock=True)
                            for attr in self.ar.data.transform_attrs[:-1]:
                                cmds.connectAttr(f"{sdk_loc}.{attr}", f"{sdk_jis}.{attr}")
                            cmds.setAttr(f"{sdk_loc_grp}.rotateX", 0)
                            cmds.setAttr(f"{sdk_loc_grp}.rotateY", 0)
                            cmds.setAttr(f"{sdk_loc_grp}.rotateZ", 0)
                    ...
                    cmds.parentConstraint(single_ctrl, jnt, maintainOffset=False, name=f"{jnt}_PaC")
                    # create scaleConstraint from ctrl to jnt:
                    cmds.scaleConstraint(single_ctrl, jnt, maintainOffset=True, name=f"{jnt}_ScC")
                # create end joint:
                cmds.select(jnt)
