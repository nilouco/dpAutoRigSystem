            ...
            if self.ar.pipeliner.checkAssetContext():
                ...
                if self.io_path:
                    ...
                    if self.first_mode: #export
                        ...
                        tagDataDic = self.importLatestJsonFile(self.get_exported_items())
                        if tagDataDic:
    ...
    def importTag(self, tagDataDic, nodeList, *args):
        ...
        if tagDataDic["tagged"]:
            if not self.defWeights.importComponentTagInfo(tagDataDic["tagged"], nodeList):
                self.fail_io(self.latestDataFile+": tagged - "+", ".join(self.defWeights.notWorkWellInfoList))
                fail = True
        ...
        if tagDataDic["influencer"]:
            if not self.defWeights.importComponentTagInfluencer(tagDataDic["influencer"]):
                self.fail_io(self.latestDataFile+": influencer - "+", ".join(self.defWeights.notWorkWellInfoList))
                fail = True
        # import falloffs
        if tagDataDic["falloff"]:
            if not self.defWeights.importComponentTagFalloff(tagDataDic["falloff"]):
                self.fail_io(self.latestDataFile+": falloff - "+", ".join(self.defWeights.notWorkWellInfoList))
                fail = True
        if not fail:
