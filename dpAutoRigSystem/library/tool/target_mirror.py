...
class TargetMirror(base.BaseLibrary):
    ...
    def run_target_mirror(self, original_model=None, target_items=None, *args):
        ...
        if orig_node and self.check_geometry(orig_node):
            # get target list:
            targets = target_items
            if not targets:
                targets = cmds.textScrollList('target_mirror_targets_tsl', query=True, allItems=True)
            if targets:
                self.ar.ui_manager.set_progress(f"Target: {self.ar.data.lang['c110_start']}", self.ar.data.lang['m055_tgtMirror'], len(targets), add_one=False, add_number=False)
                cancelled = False
                self.to_ids = []
                ...
                for item in targets:
                    ...
                    if item != orig_node and self.check_geometry(item):
                        ...
                        # apply mirror
                        cmds.setAttr(f"{bs_mirror_grp}.scale{axis}", -1)
                        ...
                        cmds.setAttr(f"{temp_to_wrap_bs}.{item}", 1)
                        ...
                        if cmds.checkBox('target_mirror_pos_cb', query=True, value=True):
                            try:
                                ...
                                axis_value = cmds.getAttr(f"{item}.translate{axis}")*(-1)
                                cmds.setAttr(f"{new_target}.translate{axis}", axis_value)
                            except:
                                pass
                        # clear undo
                        if clear_undo:
