...
class Rivet(base.BaseLibrary):
    ...
    def invert_attr_transformation(self, node_name, inv_t=True, inv_r=False, *args):
        ...
        if cmds.objExists(node_name):
            ...
            if inv_t:
                inv_t_grp = cmds.group(node_name, name=f"{node_name}_InvT_Grp")
                cmds.xform(inv_t_grp, worldSpace=True, rotatePivot=(node_pivot[0], node_pivot[1], node_pivot[2]))
                t_md = cmds.createNode('multiplyDivide', name=f"{node_name}_InvT_MD", skipSelect=True)
                self.to_ids.append(t_md)
                cmds.setAttr(f"{t_md}.input2X", -1)
                cmds.setAttr(f"{t_md}.input2Y", -1)
                cmds.setAttr(f"{t_md}.input2Z", -1)
                for axis in self.ar.data.axes:
                    cmds.connectAttr(f"{node_name}.translate{axis}", f"{t_md}.input1{axis}", force=True)
                    cmds.connectAttr(f"{t_md}.output{axis}", f"{inv_t_grp}.translate{axis}", force=True)
            if inv_r:
                inv_r_grp = cmds.group(node_name, name=f"{node_name}_InvR_Grp")
                cmds.xform(inv_r_grp, worldSpace=True, rotatePivot=(node_pivot[0], node_pivot[1], node_pivot[2]), rotateOrder="zyx")
                r_md = cmds.createNode('multiplyDivide', name=f"{node_name}_InvR_MD", skipSelect=True)
                self.to_ids.append(r_md)
                cmds.setAttr(f"{r_md}.input2X", -1)
                cmds.setAttr(f"{r_md}.input2Y", -1)
                cmds.setAttr(f"{r_md}.input2Z", -1)
                for axis in self.ar.data.axes:
                    cmds.connectAttr(f"{node_name}.rotate{axis}", f"{r_md}.input1{axis}", force=True)
                    cmds.connectAttr(f"{r_md}.output{axis}", f"{inv_r_grp}.rotate{axis}", force=True)
    ...
    def create_rivet(self, geo_to_attach, uv_set_name, items, attatch_translate, attach_rotate, add_father_grp, add_invert, inv_t, inv_r, face_to_rivet, rivet_grp_name=RIVET_GRP, ask_component=False, use_offset=True, reuse_face_to_rivet=False, *args):
        ...
        if not cmds.objExists(rivet_grp_name):
            ...
            for attr in self.ar.data.transform_attrs[:-1]:
                cmds.setAttr(f"{self.rivet_grp}.{attr}", lock=True, keyable=False, channelBox=False)
            cmds.addAttr(self.rivet_grp, longName='dpRivetGrp', attributeType='bool')
            cmds.setAttr(f"{self.rivet_grp}.dpRivetGrp", 1)
            if scalable_grp:
        ...
        if self.shapes_to_attach:
            ...
            if not add_father_grp:
                ...
                for rivet in rivets:
                    ...
                    # animated:
                    for attr in self.ar.data.transform_attrs[:-1]:
                        if cmds.listConnections(f"{rivet}.{attr}", source=True, destination=False):
                            cancel_process = True
                            break
                if cancel_process:
            ...
            dup_geo = cmds.duplicate(geo_to_attach, name=f"{geo_to_attach}_dpRivet_TEMP_Geo")[0]
            # unlock attr:
            ...
            self.temp_node = cmds.createNode('transform', name=f"{geo_to_attach}_dpRivet_TEMP_Transf", skipSelect=True)
            ...
            if self.shape_type == 'mesh':
                ...
                self.cp_node = cmds.createNode('closestPointOnMesh', name=f"{geo_to_attach}_dpRivet_TEMP_CP", skipSelect=True)
                cmds.connectAttr(f"{dup_shape}.outMesh", f"{self.cp_node}.inMesh", force=True)
                # move temp_node to cp_node position:
                cmds.connectAttr(f"{self.temp_node}.translate", f"{self.cp_node}.inPosition", force=True)
            else: #nurbsSurface
                u_range = cmds.getAttr(f"{dup_shape}.minMaxRangeU")[0]
                v_range = cmds.getAttr(f"{dup_shape}.minMaxRangeV")[0]
                # closest point on mesh node:
                self.cp_node = cmds.createNode('closestPointOnSurface', name=f"{geo_to_attach}_dpRivet_TEMP_CP", skipSelect=True)
                cmds.connectAttr(f"{dup_shape}.local", f"{self.cp_node}.inputSurface", force=True)
            ...
            for r, rivet in enumerate(rivets):
                ...
                if add_father_grp:
                    rivet = cmds.group(rivet, name=f"{rivet}_{self.rivet_grp_name}")
                ...
                fu = cmds.getAttr(f"{self.cp_node}.u")
                fv = cmds.getAttr(f"{self.cp_node}.v")
                ...
                # create follicle:
                fol_transform = cmds.createNode('transform', name=f"{rivet}_Fol", parent=self.rivet_grp, skipSelect=True)
                fol_shape = cmds.createNode('follicle', name=f"{rivet}_FolShape", parent=fol_transform, skipSelect=True)
                
                # connect geometry shape and follicle:
                if self.shape_type == 'mesh':
                    cmds.connectAttr(f"{self.shape_to_attach}.worldMesh[0]", f"{fol_shape}.inputMesh", force=True)
                    cmds.setAttr(f"{fol_shape}.mapSetName", uv_set_name, type='string')
                else: #nurbsSurface:
                    cmds.connectAttr(f"{self.shape_to_attach}.local", f"{fol_shape}.inputSurface", force=True)
                cmds.connectAttr(f"{self.shape_to_attach}.worldMatrix[0]", f"{fol_shape}.inputWorldMatrix", force=True)
                cmds.connectAttr(f"{fol_shape}.outRotate", f"{fol_transform}.rotate", force=True)
                cmds.connectAttr(f"{fol_shape}.outTranslate", f"{fol_transform}.translate", force=True)
                # put follicle in the correct place:
                cmds.setAttr(f"{fol_shape}.parameterU", fu)
                cmds.setAttr(f"{fol_shape}.parameterV", fv)
                
                # attach follicle and rivet using constraint:
                if attatch_translate and attach_rotate:
                    rivetPac = cmds.parentConstraint(fol_transform, rivet, maintainOffset=use_offset, name=f"{rivet}_PaC")[0]
                elif attatch_translate:
                    rivetPac = cmds.parentConstraint(fol_transform, rivet, maintainOffset=use_offset, name=f"{rivet}_PaC" , skipRotate=("x", "y", "z"))[0]
                elif attach_rotate:
                    rivetPac = cmds.parentConstraint(fol_transform, rivet, maintainOffset=use_offset, name=f"{rivet}_PaC" , skipTranslate=("x", "y", "z"))[0]
                
                # try to integrate to dpAutoRigSystem in order to keep the Rig as scalable:
                if master_ctrl:
                    cmds.scaleConstraint(master_ctrl, fol_transform, maintainOffset=True, name=f"{fol_transform}_ScC")
            
                # serialize network node
                self.net = cmds.createNode('network', name=f"{rivet}_Net")
                ...
                if len(items) == len(rivets) and cmds.objExists(items[r]):
                    ...
                    if not cmds.objExists(f"{items[r]}.rivetNet"):
                        ...
                        cmds.connectAttr(f"{self.net}.message", f"{items[r]}.rivetNet", force=True)
                    else:
                        ...
                        cmds.connectAttr(f"{self.net}.message", f"{items[r]}.{current_long_name}", force=True)
        ...
        if not cmds.objExists(face_to_rivet_geo_name):
            ...
            if shapes:
                ...
                if check_skin == 1:
                    ...
                    cmds.setAttr(f"{skincluster_node}.envelope", 0)
                if check_bs == 2:
                    bs_node = cmds.ls(hist_items, type='blendShape')[0]
                    cmds.setAttr(f"{bs_node}.envelope", 0)
                ...
                # Turning on nodes
                if check_skin == 1:
                    cmds.setAttr(f"{skincluster_node}.envelope", 1)
                if check_bs == 2:
                    cmds.setAttr(f"{bs_node}.envelope", 1)
    ...
    def get_face_to_rivet_geo_name(self, geo, *args):
        ...
        while done == False:
            if not cmds.objExists(f"{to_rivet_name}_FaceToRivet_{str(i).zfill(2)}_Geo"):
                done = True
            else:
                i += 1
        return f"{to_rivet_name}_FaceToRivet_{str(i).zfill(2)}_Geo"
    ...
    def create_face_to_rivet(self, controllers, geometry, grow_multiplier, *args):
        ...
        faces = cmds.ls(f"{geometry}.f[:]", flatten=True)
        face_coordinates = []
        for face in faces:
        ...
        all_faces = cmds.ls(f"{geometry}.f[*]", flatten=True)
    ...
    def apply_morph_deformer(self, morph_geo, target_geo, *args):
        ...
        cmds.setAttr(f"{morph_deformer}.morphMode", 1)
        cmds.setAttr(f"{morph_deformer}.useComponentLookup", 1)
        cmds.setAttr(f"{morph_deformer}.morphSpace", 0)
        cmds.connectAttr(f"{target_shape}.worldMesh[0]", f"{morph_deformer}.morphTarget[0]")
        component_match_node = cmds.createNode("componentMatch")
        cmds.connectAttr(f"{component_match_node}.componentLookup", f"{morph_deformer}.componentLookupList[0].componentLookup")
        morphOrigOutMesh = cmds.listConnections(f"{morph_deformer}.originalGeometry[0]", source=True, destination=False, plugs=True)[0]
        cmds.connectAttr(morphOrigOutMesh, f"{component_match_node}.inputGeometry")
        cmds.connectAttr(f"{target_orig}.outMesh", f"{component_match_node}.targetGeometry")
        #Renaming
        ...
        if '|' in to_rivet_name:
            to_rivet_name = to_rivet_name[to_rivet_name.rfind('|')+1:]
        morph_node = cmds.rename(morphs, f"{to_rivet_name}_Mrp")
        component_match_node = cmds.listConnections(f"{morph_node}.componentLookupList[0].componentLookup")[0]
        component_match_node = cmds.rename(component_match_node, f"{to_rivet_name}_CpM")
    ...
    def apply_wrap_deformer(self, wrap_geo, target_geo, *args):
        ...
        if '|' in to_rivet_name:
            to_rivet_name = to_rivet_name[to_rivet_name.rfind('|')+1:]
        wrap_node = cmds.rename(wrap_items, f"{to_rivet_name}_Wrp")
        base_shape = cmds.listConnections(f"{wrap_node}.basePoints")[0]
        base_shape = cmds.rename(base_shape, f"{to_rivet_name}_Base")
