...
class Renamer(dpBaseLibrary.BaseLibrary):
    ...
    def renamerUI(self, *args):
        ...
        self.selectRB = cmds.radioButtonGrp('selectRB', labelArray2=[self.ar.data.lang["i266_selected"], self.ar.data.lang["m216_hierarchy"]], numberOfRadioButtons=2, select=self.selOption, changeCommand=self.changeSelOption, parent=fieldsLayout)
        cmds.separator(style="in", height=20, parent=fieldsLayout)
        self.sequenceCB = cmds.checkBox('sequenceCB', label=self.ar.data.lang['m220_sequence'], changeCommand=self.sequenceCBChange, value=False, parent=fieldsLayout)
        self.sequenceTFG = cmds.textFieldGrp('sequenceTFG', label=self.ar.data.lang['m222_name'], textChangedCommand=self.nameChange, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 100)], adjustableColumn2=True, parent=fieldsLayout)
        self.startIFG = cmds.intFieldGrp('startIFG', label=self.ar.data.lang['c110_start'], changeCommand=self.refreshPreview, value1=self.start, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 100)], adjustableColumn2=True, parent=fieldsLayout)
        self.paddingIFG = cmds.intFieldGrp('paddingIFG', label=self.ar.data.lang['m221_padding'], changeCommand=self.refreshPreview, value1=self.padding, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 100)], adjustableColumn2=True, parent=fieldsLayout)
        cmds.separator(style="in", height=20, parent=fieldsLayout)
        ...
        self.prefixCB = cmds.checkBox('prefixCB', label=self.ar.data.lang['i144_prefix'], changeCommand=self.refreshPreview, value=False, parent=prePosLayout)
        self.suffixCB = cmds.checkBox('suffixCB', label=self.ar.data.lang['m217_suffix'], changeCommand=self.refreshPreview, value=False, parent=prePosLayout)
        ...
        cmds.separator(style="in", height=20, parent=fieldsLayout)
        self.searchReplaceCB = cmds.checkBox('searchReplaceCB', label=self.ar.data.lang['m218_search']+" - "+self.ar.data.lang['m219_replace'], changeCommand=self.searchReplaceCBChange, value=False, parent=fieldsLayout)
        self.searchTFG = cmds.textFieldGrp('searchTFG', label=self.ar.data.lang['i036_from'], textChangedCommand=self.searchChange, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 136)], adjustableColumn2=True, parent=fieldsLayout)
        self.replaceTFG = cmds.textFieldGrp('replaceTFG', label=self.ar.data.lang['i037_to'], textChangedCommand=self.searchChange, columnAlign=[(1, "right"), (2, "right")], columnWidth=[(1, 30), (2, 136)], adjustableColumn2=True, parent=fieldsLayout)
        # loaded items
        itemsLayout = cmds.columnLayout('itemsLayout', adjustableColumn=True, width=300, parent=mainLayout)
        cmds.text(label=self.ar.data.lang['m223_preview'], align="center", height=20, font="boldLabelFont", parent=itemsLayout)
        ...
        cmds.text(label=self.ar.data.lang['i276_current'], parent=scrollsLayout)
        cmds.text(label=self.ar.data.lang['m224_rename']+" "+self.ar.data.lang['i037_to'], parent=scrollsLayout)
        ...
        cmds.button('runRenamerBT', label=self.ar.data.lang['m224_rename'], command=self.runRenamerByUI, parent=footerLayout)
        # calling UI:
