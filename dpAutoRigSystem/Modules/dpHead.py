    ...
    def rigModule(self, *args):
        ...
        if cmds.objExists(self.moduleGrp):
            ...
            for s, side in enumerate(sideList):
                ...
                dpUtils.originedFrom(objName=self.headCtrl, attrString=self.cvHeadLoc)
                ...
                if self.addArticJoint:
                    ...
                    if self.addCorrective:
                        ...
                        for n in range(0, self.nJoints):
                            ...
                            self.setupJcrControls(articJntList, s, jointLabelAdd, neckCtrlBaseName+"_"+str(n), correctiveNetList, neckHeadCalibratePresetList, invertList)
                            if s == 1:
                                if self.addFlip:
                        ...
                        self.setupJcrControls(articJntList, s, jointLabelAdd, side+self.userGuideName+"_"+self.langDic[self.langName]['c024_head'], headCorrectiveNetList, headCalibratePresetList, invertList)
                        if s == 1:
                            if self.addFlip:
                                cmds.setAttr(articJntList[0]+".scaleX", -1)
                                cmds.setAttr(articJntList[0]+".scaleY", -1)
