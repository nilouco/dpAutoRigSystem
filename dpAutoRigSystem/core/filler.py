...
class UIFiller:
    ...
    def populate_library(self, item, folder, layout, columns=5):
        if cmds.layout(layout, query=True, exists=True):
            ...
            cmds.image(f"{item.name}_img", image=self.ar.data.icon[icon_name], width=32, parent=module_layout)
            # standard
            if folder == self.ar.data.standard_folder:
                cmds.button(f"{item.name}_bt", label=self.ar.data.lang[item.title], height=32, command=partial(self.ar.maker.create_raw_guide, item.name), parent=module_layout)
            # templates
            elif folder == self.ar.data.template_folder:
                if item.title == self.ar.data.template_default:
                    cmds.button(f"{item.name}_bt", label=item.name.capitalize().replace('_', '\n'), height=32, command=item.build_template, parent=module_layout)
                else:
                    cmds.button(f"{item.name}_bt", label=self.ar.data.lang[item.title], height=32, command=item.build_template, parent=module_layout)
            # tools
            elif folder == self.ar.data.tools_folder:
                cmds.button(f"{item.name}_bt", label=self.ar.data.lang[item.title], height=32, width=200, command=item.build_tool, parent=module_layout)
            ...
            # info icon
            cmds.iconTextButton(f"{item.name}_itb", image=self.ar.data.icon['info'], height=30, width=30, style='iconOnly', command=partial(self.ar.logger.infoWin, item.title, item.description, None, 'center', 305, 250, wiki=item.wiki), parent=module_layout)
            if folder in self.rebuilder_folders:
        ...
        for n in namespaces:
            ...
            if n_partitions[1] != '':
                ...
                if module in valid_module_names:
                    ...
                    curGuideName = f"{valid_module_names[index]}__{userSpecName}:{self.ar.data.guide_base_name}"
        ...
        if self.ar.data.ui_state:
            ...
            if choose_joint == 'allJoints':
                ...
                for joint_node in all_joints:
                    if cmds.objExists(f"{joint_node}.{self.ar.data.base_name}joint"):
            ...
            for geo_type in ['mesh', 'nurbsSurface', 'subdiv']:
                ...
                if all_geos:
                    for mesh in all_geos:
                        if cmds.getAttr(f"{mesh}.intermediateObject") == 0:
                            transforms = cmds.listRelatives(mesh, parent=True, fullPath=True, type='transform')
                            # do not add ribbon nurbs plane to the list:
                            if transforms and not cmds.objExists(f"{transforms[0]}.{self.ar.skin.ignore_skinning_attr}") and not transforms[0] in geos:
    ...
    def check_imported_guides(self, ask_user=True, *args):
        ...
        """
        imported_namespaces = []
        current_custom_names = [cmds.getAttr(f"{guideModule.guide_base}.customName") for guideModule in self.ar.utils.get_guides_to_rig()]
        cmds.namespace(setNamespace=':')
        ...
        if namespaces:
            ...
            if imported_namespaces:
                ...
                for name in imported_namespaces:
                    if cmds.objExists(f"{name}:Guide_Base.customName"):
                        n = 1
                        old_custom_name = cmds.getAttr(f"{name}:Guide_Base.customName")
                        if old_custom_name:
                            base_name = old_custom_name
                            while old_custom_name in current_custom_names:
                                old_custom_name = f"{base_name}{n}"
                                n += 1
                            cmds.setAttr(f"{name}:Guide_Base.customName", old_custom_name, type='string')
