...
DP_REBUILDERSTARTCLASS_VERSION = 1.0


class RebuilderStartClass(object):
    ...
    def changeActive(self, value, *args):
        ...
        self.active = value
        if self.ui:
            cmds.checkBox(self.rebuilderCB, edit=True, value=value)
            cmds.button(self.exportBT, edit=True, enable=value)
            cmds.button(self.importBT, edit=True, enable=value)
    ...
    def updateButtonColors(self, *args):
        ...
        """
        if self.ui:
            if cmds.button(self.exportBT, exists=True):
                if self.checkedObjList:
                    if self.verifyMode:
                        if True in self.foundIssueList:
                            cmds.button(self.exportBT, edit=True, backgroundColor=ISSUE_COLOR)
                            cmds.button(self.importBT, edit=True, backgroundColor=WARNING_COLOR)
                        else:
                            cmds.button(self.exportBT, edit=True, backgroundColor=CHECKED_COLOR)
                            cmds.button(self.importBT, edit=True, backgroundColor=DEFAULT_COLOR)
                    else: #fix
                        if False in self.resultOkList:
                            cmds.button(self.exportBT, edit=True, backgroundColor=WARNING_COLOR)
                            cmds.button(self.importBT, edit=True, backgroundColor=ISSUE_COLOR)
                        else:
                            cmds.button(self.exportBT, edit=True, backgroundColor=DEFAULT_COLOR)
                            cmds.button(self.importBT, edit=True, backgroundColor=CHECKED_COLOR)
                else:
                    if self.verifyMode:
                        cmds.button(self.exportBT, edit=True, backgroundColor=CHECKED_COLOR)
                        cmds.button(self.importBT, edit=True, backgroundColor=DEFAULT_COLOR)
